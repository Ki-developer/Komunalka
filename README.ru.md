...
| 🧮 **Расшифровка «прочих»** | Прочие расходы можно расписать своими строками — вывоз мусора, содержание дома, домофон. Сама сумма по-прежнему считается как остаток квитанции. |
...
| ⏳ **Защита от подбора** | После 5 неверных паролей вход встаёт на паузу: сначала на 5 минут, затем дольше. |
...
- **Блокировка входа.** После 5 неверных паролей подряд вход закрывается на 5 минут, затем на 15 минут и на час. Счётчик хранится у Google для вашего аккаунта, поэтому перезагрузка страницы или режим инкогнито его не сбрасывают. Снять раньше: в таблице меню **Комуналка → Снять блокировку входа**.
  ...
  "rows": [
    {
      ...
      "paid": 1296.61,
      "rent": 10000,
      "otherItems": [{ "name": "Вывоз мусора", "sum": 120 }],
      "payDate": "2026-09-21",
      "payMethod": "card",
...
| `otherItems` | расшифровка прочих расходов: `[{ "name": "Вывоз мусора", "sum": 120 }]` — необязательно, сумма всё равно считается как остаток |
| `rent` | квартплата за месяц |
| `payMethod` | как платили: `card` (картой / переводом) или `cash` (наличными) — необязательно |
| `payCard` | куда платили: карта или реквизиты — необязательно |
...
> [!IMPORTANT]
> Обновляйте **оба** файла: не только `Index.html`, но и `Code.gs`. Блокировка входа и запоминание языка работают через `Code.gs`.
...
| *«Слишком много неверных попыток»* | Дождитесь окончания отсчёта или откройте таблицу → меню **Комуналка → Снять блокировку входа**. |
