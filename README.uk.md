<div align="center">

<img src="docs/logo.svg" width="88" alt="Логотип Комуналки">

# Комуналка

**Зашифрований облік лічильників і оплат для тих, хто орендує житло.**<br>
Працює у вашому власному Google-акаунті, на телефоні й комп’ютері, українською, російською та англійською.

![Google Apps Script](https://img.shields.io/badge/Google%20Apps%20Script-%D0%B2%D0%B5%D0%B1%D0%B4%D0%BE%D0%B4%D0%B0%D1%82%D0%BE%D0%BA-4285F4?logo=google&logoColor=white)
![Шифрування](https://img.shields.io/badge/%D1%88%D0%B8%D1%84%D1%80%D1%83%D0%B2%D0%B0%D0%BD%D0%BD%D1%8F-AES--256--GCM-8B5CF6)
![Сервер](https://img.shields.io/badge/%D1%81%D0%B5%D1%80%D0%B2%D0%B5%D1%80-%D0%BD%D0%B5%20%D0%BF%D0%BE%D1%82%D1%80%D1%96%D0%B1%D0%B5%D0%BD-2ea44f)
![Мови](https://img.shields.io/badge/%D0%BC%D0%BE%D0%B2%D0%B8-UA%20%7C%20RU%20%7C%20EN-orange)
[![Ліцензія: MIT](https://img.shields.io/badge/%D0%BB%D1%96%D1%86%D0%B5%D0%BD%D0%B7%D1%96%D1%8F-MIT-blue)](LICENSE)

[English](README.md) · [Русский](README.ru.md) · **Українська**

<img src="docs/screenshots/uk/overview.webp" alt="Головний екран: що внести далі, ключові цифри та графік по місяцях" width="920">

</div>

---

## Зміст

- [Навіщо](#навіщо)
- [Можливості](#можливості)
- [Скриншоти](#скриншоти)
- [Як захищені дані](#як-захищені-дані)
- [Встановлення (≈10 хвилин)](#встановлення-10-хвилин)
- [Як користуватися](#як-користуватися)
- [Доступ для родини](#доступ-для-родини)
- [Завантаження історії](#завантаження-історії)
- [Резервні копії та відновлення](#резервні-копії-та-відновлення)
- [Оновлення програми](#оновлення-програми)
- [Якщо щось не працює](#якщо-щось-не-працює)
- [Склад проєкту](#склад-проєкту)
- [Для розробників](#для-розробників)
- [Ліцензія](#ліцензія)
- [Автор](#автор)

---

## Навіщо

Орендована квартира — це щомісячний ритуал: зняти показання, перевірити квитанцію, переказати гроші разом із квартплатою і десь це все записати. Зазвичай — у таблицю, яка з часом перетворюється на важко читабельне полотно.

Комуналка перетворює таку таблицю на невеликий застосунок:

- **підказує, що час зробити** — за *вашим* графіком;
- **рахує** споживання й суму одразу, щойно ви ввели показання;
- **знаходить помилки** — від’ємне споживання, незвичні стрибки, повторювані номери оплат;
- зберігає **фото й PDF квитанцій** поруч із потрібним місяцем;
- і все це **шифрується на вашому пристрої** ще до надсилання в Google.

## Можливості

| | |
|---|---|
| 🗓 **Свій графік** | Коли вносити кожен лічильник і коли оплачувати квитанцію (наприклад, вода — 20-го, електроенергія й газ — 1-го числа наступного місяця). Головний екран показує, що час, що прострочено і що попереду. |
| ⚡ **Миттєвий розрахунок** | Споживання, сума за кожним лічильником, порівняння з минулим місяцем і тим самим місяцем рік тому, «звичайний» рівень за 12 місяців. |
| 🧾 **Квитанції та оплати** | Опалення, скільки сплачено, квартплата, дата й номер операції. «Інші витрати» рахуються автоматично як залишок квитанції. |
| 🧮 **Розшифровка «інших»** | Інші витрати можна розписати своїми рядками — вивіз сміття, утримання будинку, домофон. Сама сума так само рахується як залишок квитанції. |
| 📎 **Файли** | Фото й PDF квитанцій і чеків із коментарями. Фото зменшуються автоматично. |
| 📊 **Аналіз** | Графіки по місяцях і «рік до року», підсумки за роками, спільна шкала для гарячої та холодної води, автоматичні перевірки. |
| 💱 **Валюта** | За замовчуванням гривня; у налаштуваннях можна обрати євро, долар, злотий, фунт та інші. |
| 🔁 **Зміна тарифів і квартплати** | Значення переносяться з минулого місяця. Змінили один раз — програма запропонує оновити й наступні місяці. |
| 🔔 **Нагадування** | Повторювані нагадування в Google Календар в один дотик або файл `.ics` для iPhone / Outlook. |
| 🌍 **Три мови** | Українська, російська, англійська — перемикаються будь-коли. |
| 📱 **Зручно з телефона** | Значок на головному екрані, нижнє меню, кнопка «Назад» браузера працює всередині програми. |
| 🔐 **Приватність** | Шифрування AES-256 вашим паролем, автоблокування, доступ лише для вашого Google-акаунта. |
| ⏳ **Захист від підбору** | Після 5 невірних паролів вхід стає на паузу: спочатку на 5 хвилин, потім довше. |
| 👨‍👩‍👧 **Доступ для родини** | Програмою можуть користуватися всі, хто живе в квартирі — кожен входить під своїм Google-акаунтом, дані залишаються зашифрованими. Див. [Доступ для родини](#доступ-для-родини). |

## Скриншоти

> На всіх скриншотах — **вигадані демо-дані**.

<table>
<tr>
<td width="50%"><img src="docs/screenshots/uk/entry.webp" alt="Введення показань"><br><sub><b>Введення показань.</b> Лічильники згруповані за датами внесення; споживання, сума й порівняння з’являються просто під час введення.</sub></td>
<td width="50%"><img src="docs/screenshots/uk/entry-bill.webp" alt="Квитанція, оплата та файли"><br><sub><b>Квитанція та оплата.</b> Прикріпіть фото квитанції, додайте коментар, перегляньте повний розрахунок за місяць.</sub></td>
</tr>
<tr>
<td><img src="docs/screenshots/uk/history.webp" alt="Таблиця історії"><br><sub><b>Історія.</b> Таблиця як в Excel: споживання, гроші або показання й тарифи, з підсумками за роками.</sub></td>
<td><img src="docs/screenshots/uk/analysis.webp" alt="Графіки аналізу"><br><sub><b>Аналіз.</b> Споживання по місяцях із середнім за 12 місяців; режим «рік до року»; підсумки за роками.</sub></td>
</tr>
<tr>
<td><img src="docs/screenshots/uk/schedule.webp" alt="Налаштування графіка"><br><sub><b>Графік.</b> Дати задаються на прикладі поточного місяця — зведення нижче показує, як це працюватиме.</sub></td>
<td><img src="docs/screenshots/uk/lock.webp" alt="Екран входу"><br><sub><b>Вхід.</b> Пароль розшифровує дані на вашому пристрої й нікуди не надсилається.</sub></td>
</tr>
</table>

<p align="center">
<img src="docs/screenshots/uk/mobile-overview.webp" alt="Телефон: головний екран" width="260">
&nbsp;&nbsp;
<img src="docs/screenshots/uk/mobile-entry.webp" alt="Телефон: введення показань" width="260">
<br><sub>На телефоні: головний екран і форма введення.</sub>
</p>

## Як захищені дані

```mermaid
flowchart LR
    A["📱 Ваш телефон / комп’ютер<br/>застосунок Комуналка"] -- "зашифровані записи й файли" --> B["Google Apps Script<br/>(Code.gs)"]
    B --> C[("Google Таблиця<br/>на вашому Google Диску<br/>лише шифр")]
    P["🔑 Пароль"] -. "залишається на пристрої" .-> A
```

- **Шифрування відбувається в браузері.** З пароля отримується ключ (PBKDF2-SHA-256, 310 000 ітерацій). Ним захищено випадковий 512-бітний *ключ даних*, а вже ключ даних шифрує кожен місяць, налаштування й кожен файл алгоритмом **AES-256-GCM**.
- **Google зберігає лише шифр.** У таблиці — нечитабельні рядки: ані показань, ані сум, ані приміток, ані фото у відкритому вигляді. Приховані навіть назви місяців: ідентифікатори записів — це HMAC.
- **Доступ лише у вас.** Вебдодаток розгортається з налаштуваннями *«Виконувати як: Я / Хто має доступ: Лише я»* — відкрити його може тільки ваш Google-акаунт.
- **Зміна пароля миттєва** — перешифровується лише невеликий ключ, а не всі дані.
- **Автоблокування** через 5–60 хвилин бездіяльності (за замовчуванням 10).
- **Блокування входу.** Після 5 невірних паролів поспіль вхід закривається на 5 хвилин, потім на 15 хвилин і на годину. Лічильник зберігається в Google для вашого акаунта, тому перезавантаження сторінки чи режим інкогніто його не скидають. Зняти раніше: у таблиці меню **Комуналка → Снять блокировку входа**.
- **Жодного стороннього коду.** Увесь застосунок — один HTML-файл без зовнішніх скриптів.

> [!WARNING]
> **Пароль неможливо відновити.** Без нього дані не розшифрує ніхто — зокрема й ви. Запишіть пароль у надійному місці й час від часу робіть зашифровану копію.

> [!NOTE]
> Вивантаження в CSV і файл для завантаження історії **не** зашифровані. Видаляйте їх, коли вони більше не потрібні.

## Встановлення (≈10 хвилин)

Знадобиться Google-акаунт. Найзручніше робити це з комп’ютера.

**0. Завантажте файли.** На цій сторінці натисніть **Code → Download ZIP** і розпакуйте архів. Потрібні `Code.gs` та `Index.html`.

> [!TIP]
> Відкривайте файли звичайним текстовим редактором: права кнопка → *Відкрити за допомогою* → *Блокнот*. Якщо двічі клацнути по `Index.html`, він відкриється в браузері як програма, а не як текст.

1. **Створіть таблицю.** Відкрийте [sheets.google.com](https://sheets.google.com), створіть порожню таблицю й назвіть її, наприклад, *Комуналка*.
2. **Відкрийте редактор скриптів.** У меню таблиці: **Розширення → Apps Script**. Перейменуйте проєкт (ліворуч угорі, *Проєкт без назви*) на *Комуналка*.
3. **Додайте серверну частину.** У редакторі вже є файл `Код.gs`. Видаліть у ньому все й вставте повністю вміст `Code.gs` із цього репозиторію.
4. **Додайте застосунок.** Біля *Файли* натисніть **+ → HTML** і назвіть файл рівно **`Index`** (з великої літери, `.html` редактор додасть сам). Видаліть у ньому все й вставте повністю вміст `Index.html`. Вставлений текст має закінчуватися на `</html>`.
5. **Збережіть** проєкт (💾 або `Ctrl+S`).
6. **Розгорніть.** **Розгорнути → Нове розгортання**, шестерня ⚙ біля *Вибрати тип* → **Вебдодаток**:
   - *Опис*: `Комуналка`
   - *Виконувати як*: **Я**
   - *Хто має доступ*: **Лише я**

   Натисніть **Розгорнути**.
7. **Надайте доступ.** Google попросить дозвіл → **Надати доступ** → оберіть свій акаунт. З’явиться *«Google не перевірив цей додаток»* — для власного скрипта це нормально. Натисніть **Додатково → Перейти на сторінку «Комуналка» (небезпечно) → Дозволити**. Скрипт отримує доступ **лише до цієї однієї таблиці**.
8. **Скопіюйте URL вебдодатка** (закінчується на `/exec`) і додайте в закладки. Це і є програма.
9. **Відкрийте посилання** й придумайте пароль (не коротше 8 символів).
10. **Налаштування** (вкладка *Ще*): оберіть валюту, за бажання впишіть номер картки власника, перевірте графік і додайте нагадування в календар.

> Назви кнопок Google можуть трохи відрізнятися залежно від мови інтерфейсу вашого акаунта.

**На телефоні:** відкрийте те саме посилання `/exec` у Chrome (Android) або Safari (iPhone) під тим самим Google-акаунтом → меню браузера → **Додати на головний екран**.

## Як користуватися

1. **У день внесення** (головний екран нагадає) відкрийте **Внести**, впишіть показання й натисніть **Зберегти**. Воду, електроенергію й газ можна вносити в різні дні — просто знову відкрийте той самий місяць.
2. **Коли прийшла квитанція і ви оплатили**, відкрийте цей місяць і заповніть *Опалення*, *Сплачено за комуналку*, дату й номер операції. Прикріпіть фото квитанції.
3. **Змінився тариф або квартплата?** Впишіть нове значення в **першому місяці**, з якого воно діє. Далі воно підставиться саме; якщо в наступних місяцях уже стоїть старе значення, програма запропонує замінити й там.
4. **Замінили лічильник?** Позначте *«Лічильник замінили цього місяця»* і вкажіть початкове показання нового.
5. **Дивіться результати:** *Історія* — усе у вигляді таблиці (натисніть на рядок, щоб виправити), *Аналіз* — графіки й перевірки.

## Доступ для родини

Однією Комуналкою можуть користуватися кілька людей — наприклад, усі, хто живе в квартирі. Усі працюють з одними й тими самими даними, а входять **кожен під своїм Google-акаунтом**.

### Як це влаштовано

| | Лише ви (за замовчуванням) | Родина |
|---|---|---|
| *Виконувати як* | Я | Користувач, який відкриває вебдодаток |
| *Хто має доступ* | Лише я | Усі, хто має обліковий запис Google |
| Хто може завантажити дані | лише ви | лише ті, кому відкрито доступ до таблиці |
| Пароль програми | ваш | один на всю родину |

Apps Script не вміє дозволяти вебдодаток списку конкретних людей. Тому програма запускається **від імені того, хто її відкриває**, а доступ до даних визначається звичайним доступом до Google Таблиці. Людина, у якої є посилання, але немає доступу до таблиці, нічого не завантажить — а без пароля програми дані все одно залишаються зашифрованими.

### Крок 1. Відкрийте доступ до таблиці

1. Відкрийте таблицю *Комуналка* → **Налаштування доступу** (праворуч угорі).
2. Впишіть Gmail кожного члена родини й оберіть роль **«Редактор»**.
3. Натисніть **«Надіслати»**. Відкривати саму таблицю їм не потрібно.

### Крок 2. Змініть розгортання

1. У таблиці: **Розширення → Apps Script**.
2. **Розгорнути → Керувати розгортаннями** → ✏️ **Редагувати**.
3. *Виконувати як*: **Користувач, який відкриває вебдодаток** (англійською *User accessing the web app*).
4. *Хто має доступ*: **Усі, хто має обліковий запис Google** (*Anyone with Google account*).
5. *Версія*: **Нова версія** → **Розгорнути**.

Посилання `/exec` залишиться тим самим, закладки продовжать працювати. Google може ще раз попросити надати доступ — так само, як під час встановлення.

> Назви пунктів можуть трохи відрізнятися залежно від мови вашого Google-акаунта — у дужках вказано англійські.

### Крок 3. Запросіть родину

1. Надішліть їм посилання `/exec`.
2. **Пароль програми повідомте особисто** — не в тому самому повідомленні, що й посилання.

### Крок 4. Перший вхід (для кожного члена родини)

1. Відкрийте посилання, **увійшовши в той Google-акаунт, якому відкрили доступ до таблиці**.
2. Google попросить дозвіл → **Надати доступ** → оберіть акаунт → *«Google не перевірив цей додаток»* → **Додатково → Перейти на сторінку «Комуналка» (небезпечно) → Дозволити**. Це той самий скрипт, і доступ у нього лише до цієї однієї таблиці.
3. Угорі може з’явитися сіра смуга *«Цей додаток створено іншим користувачем, а не Google»* — це нормально.
4. Введіть пароль програми.
5. На телефоні: меню браузера → **Додати на головний екран**.

### Безпека та корисні правила

- **Надавайте доступ лише тим, кому довіряєте.** Редактори можуть відкрити таблицю (там лише шифр) і код скрипта — і випадково чи навмисно їх зіпсувати.
- **Один пароль на всіх.** Налаштування — картка, графік, валюта, автоблокування — теж спільні. Лише мова інтерфейсу обирається на кожному пристрої окремо.
- **Одночасне редагування.** Якщо двоє одночасно редагують **той самий місяць**, збережеться те, що зберегли останнім. Різні місяці й файли одне одному не заважають. Коли ви повертаєтеся в програму, вона підтягує свіжі дані.
- **Резервні копії** як і раніше робить власник: *Ще → Завантажити зашифровану копію*.

### Як відключити людину

1. Таблиця → **Налаштування доступу** → біля людини оберіть **«Видалити доступ»**.
2. У програмі: **Ще → Змінити пароль** і повідомте новий пароль іншим.

### Як повернутися до режиму «лише я»

**Розгорнути → Керувати розгортаннями** → ✏️ → *Виконувати як*: **Я**, *Хто має доступ*: **Лише я** → *Версія*: **Нова версія** → **Розгорнути**. За бажання закрийте й доступ до таблиці.

### Якщо не виходить

| Проблема | Рішення |
|---|---|
| *«Не вдалося завантажити дані»* або помилка доступу | Таблицю не відкрито цьому акаунту, або в браузері обрано інший Google-акаунт. |
| *«Не вдається відкрити файл»* (часто на телефонах) | Браузер відкриває посилання під іншим Google-акаунтом. Перевірте в режимі інкогніто. На Android використовуйте браузер, у якому ввійшли лише потрібним акаунтом; на iPhone вимкніть **Параметри → Safari → Заборонити стеження між сайтами** або відкрийте посилання в Chrome чи в застосунку Google. |
| Посилання відкривається всередині Telegram / Viber | Натисніть **«Відкрити в браузері»**. |
| Хтось бачить стару версію | Розгортання не оновлено до **нової версії**. |

## Завантаження історії

Якщо ви вели облік у таблиці, його можна завантажити одразу: **Ще → Завантажити історію з Excel (.json)** або кнопкою на порожньому головному екрані. Формат — простий JSON, приклад: [`docs/import-example.json`](docs/import-example.json).

```json
{
  "app": "komunalka-import",
  "version": 1,
  "settings": { "rent": 10000 },
  "rows": [
    {
      "month": "2026-08",
      "readings": { "el": 22817.0, "gas": 223.9381, "hw": 53.191, "cw": 404.568 },
      "tariffs":  { "el": 4.32, "gas": 7.95689, "hw": 97.89, "cw": 30.384 },
      "status":   { "el": "ok" },
      "heating": 0,
      "paid": 1296.61,
      "rent": 10000,
      "otherItems": [{ "name": "Вивіз сміття", "sum": 120 }],
      "payDate": "2026-09-21",
      "payMethod": "card",
      "payInfo": "Ref. 489724997658",
      "note": "Будь-який текст"
    }
  ]
}
```

| Поле | Що означає |
|---|---|
| `month` | `РРРР-ММ` — місяць, до якого належать показання |
| `readings` | показання: `el` електроенергія, `gas` газ, `hw` гаряча вода, `cw` холодна вода |
| `tariffs` | ціна за одиницю для кожного лічильника |
| `status` | звірка з бланком: `ok`, `gap` (пропуск у бланку), `noblank` (бланка немає), `wrong` (невірні дані), `est` (розрахункове) |
| `newMeter` | `{ "cw": 0 }` — лічильник замінили; споживання рахується від цього початкового показання |
| `heating` | сума за опалення за квитанцією |
| `other` | інші витрати (необов’язково; якщо немає — рахуються як залишок від `paid`) |
| `paid` | скільки сплачено за комуналку (без квартплати) |
| `otherItems` | розшифровка інших витрат: `[{ "name": "Вивіз сміття", "sum": 120 }]` — необов’язково, сума все одно рахується як залишок |
| `rent` | квартплата за місяць |
| `payMethod` | як платили: `card` (карткою / переказом) або `cash` (готівкою) — необов’язково |
| `payCard` | куди платили: картка або реквізити — необов’язково |
| `payDate`, `payInfo`, `note` | дата оплати (`РРРР-ММ-ДД`), інформація про операцію, примітка |

Файл для завантаження **не** зашифрований — видаліть його після завантаження.

## Резервні копії та відновлення

- **Зашифрована копія:** *Ще → Завантажити зашифровану копію*. Її можна зберігати будь-де (Google Диск, флешка) — без пароля її не прочитати. Фото й PDF до копії не входять, вони залишаються в таблиці. Саму таблицю теж можна скопіювати на Google Диску.
- **Відновлення:** *Ще → Відновити з копії* (потрібен пароль, з яким копію було зроблено).
- **Забули пароль?** Відкрийте таблицю → меню **Комуналка → Удалить все данные** (*видалити всі дані*), потім задайте новий пароль і відновіть копію або знову завантажте історію.

## Оновлення програми

Вставте нові `Code.gs` / `Index.html` у редактор Apps Script → **Зберегти** → **Розгорнути → Керувати розгортаннями** → ✏️ → *Версія*: **Нова версія** → **Розгорнути**. Посилання `/exec` залишиться тим самим.

> [!IMPORTANT]
> Оновлюйте **обидва** файли: не лише `Index.html`, а й `Code.gs`. Блокування входу та запам’ятовування мови працюють через `Code.gs`.

> [!TIP]
> Найнадійніше копіювати так: відкрийте файл на GitHub ([Index.html](Index.html)) і натисніть **Copy raw file** (значок із двома квадратиками над кодом). У редакторі натисніть `Ctrl+A`, `Ctrl+V`, `Ctrl+S`, потім `Ctrl+End`: останній рядок має бути `</html>`.

## Якщо щось не працює

| Проблема | Рішення |
|---|---|
| *«Не вдається відкрити файл»* або інша помилка Google | У браузері виконано вхід у кілька Google-акаунтів. Відкрийте посилання в режимі інкогніто або залиште один акаунт. На iPhone також вимкніть **Параметри → Safari → Заборонити стеження між сайтами**. |
| Програма показує стару версію | Код збережено, але не розгорнуто **нову версію** — див. «Оновлення програми». |
| *«Забагато невірних спроб»* | Дочекайтеся кінця відліку або відкрийте таблицю → меню **Комуналка → Снять блокировку входа**. |
| Нескінченне *«Завантаження…»* або повідомлення *«Програма не запустилася: …»* | Оновіть `Index.html` до останньої версії (див. «Оновлення програми»): версії, випущені до 22 вересня 2026 року, могли ламатися в Apps Script — подробиці в розділі [Для розробників](#для-розробників). Якщо це сталося з останньою версією, скопіюйте файл заново через **Copy raw file**, перевірте, що він закінчується на `</html>`, і розгорніть нову версію. На екрані помилки видно точний рядок — додайте скриншот, коли [створюєте issue](https://github.com/Ki-developer/Komunalka/issues). |
| Порожня сторінка після вставлення `Index.html` | Файл вставлено не повністю або він називається не рівно `Index`. |
| Хочу спробувати без Google | Відкрийте `Index.html` просто в браузері на комп’ютері. Програма запрацює в *локальному режимі*: дані лише в цьому браузері, без телефона й без файлів. |
| Іконка вкладки | Задається в `Code.gs` (`FAVICON_URL`). Apps Script приймає лише публічне посилання на картинку `.png`. |

## Склад проєкту

```
Code.gs                    серверна частина Apps Script: зберігає зашифровані записи й файли в таблиці
Index.html                 увесь застосунок: інтерфейс, графіки, шифрування, переклади — без зовнішніх залежностей
docs/import-example.json   вигадана історія у форматі для завантаження
docs/screenshots/          скриншоти (демо-дані) англійською, російською та українською
docs/logo.svg              логотип
LICENSE                    ліцензія MIT
```

## Для розробників

**Apps Script може вирізати `//…` зі сторінки.** Коли `Index.html` великий, Apps Script іноді віддає його без усього, що стоїть від `//` до кінця рядка, — навіть усередині рядків. Наприклад, `'https://calendar…'` перетворювався на `'https:`, увесь скрипт падав з помилкою *Invalid or unexpected token*, і програма зависала на «Завантаженні…». Тому в `Index.html` не має бути `//` і `/*` ніде, крім справжніх коментарів:

- адреси всередині коду пишіть як `'https:\/\/…'`;
- у SVG усередині data-URI використовуйте `%2F%2F` (`xmlns='http:%2F%2Fwww.w3.org%2F2000%2Fsvg'`);
- жодних регулярних виразів на кшталт `/^image\//` — використовуйте `startsWith('image/')`.

Швидка перевірка після змін: зробіть копію без коментарів і відкрийте її в браузері — має з’явитися екран входу з перемикачем RU / UA / EN.

```bash
python -c "import re; s = open('Index.html', encoding='utf-8').read(); s = re.sub(r'//[^\n]*', '', s); s = re.sub(r'/\*.*?\*/', '', s, flags=re.S); open('stripped.html', 'w', encoding='utf-8').write(s)"
```

В Apps Script сторінка вставляється через `document.write`, тому номери рядків у помилках рахуються від початку основного `<script>`, а не від початку файлу.

## Ліцензія

[MIT](LICENSE) — можна вільно використовувати, копіювати, змінювати й поширювати, зокрема в комерційних цілях, за умови збереження повідомлення про авторські права. Програма надається «як є», без жодних гарантій.

## Автор

**Kyrylo Vasylenko**

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Kyrylo%20Vasylenko-0A66C2?logo=linkedin&logoColor=white)](https://www.linkedin.com/in/xg6nwh5sfw-kyr-vas/)

LinkedIn: [https://www.linkedin.com/in/xg6nwh5sfw-kyr-vas/](https://www.linkedin.com/in/xg6nwh5sfw-kyr-vas/)

Питання, ідеї та повідомлення про помилки — створіть issue або напишіть у LinkedIn.

<div align="center">
<sub>Зроблено для справжнього щомісячного ритуалу орендаря.</sub>
</div>
